...
const SUGGESTIONS = [
  'PS5 racing games under $70',
  'Something good for couch co-op',
  ...
  return (
    <>
      <Button
        size="icon"
        onClick={() => setOpen((v) => !v)}
        className="fixed bottom-6 right-6 z-50 size-14 rounded-full shadow-glow"
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 flex h-[34rem] max-h-[75vh] w-[22rem] flex-col overflow-hidden rounded-2xl border border-primary/20 bg-card shadow-2xl animate-fade-in">
          <header className="flex items-center gap-2 border-b border-border bg-primary/5 px-4 py-3">
            <Sparkles className="size-4 text-primary" />
            <div className="leading-tight">
              <p className="text-sm font-semibold">RiftDrop Assistant</p>
              <p className="text-xs text-muted-foreground">Find games · check orders</p>
            </div>
          </header>
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        Looking for a PS5 drop, a co-op classic, or an order update? Try:
      </p>
      <div className="flex flex-col gap-2">
        {SUGGESTIONS.map((s) => (
          <button
            ...
            type="button"
            onClick={() => onPick(s)}
            className="cursor-pointer rounded-lg border border-border bg-secondary/40 px-3 py-2 text-left text-sm transition-colors hover:border-primary/40 hover:bg-secondary"
          >
            {s}
          </button>
        ))}
...
function ProductCard({ product }: { product: ChatProductCard }) {
  return (
    <Link
      ...
      className="flex items-center gap-3 rounded-lg border border-border bg-background p-2 transition-colors hover:border-primary/40"
    >

...
export class ProductsService {
  ...
  async findManyForStorefront(query: ProductQueryDto): Promise<Paginated<unknown>> {
    ...
    const categoryIds = await this.resolveCategoryScope(query.categoryId);
    const where: Prisma.ProductWhereInput = {
      deletedAt: null,
  ...
  /** Include a category and its direct children so parent chips (e.g. PS5 Games) match leaf products. */
  private async resolveCategoryScope(categoryId?: string): Promise<string[] | undefined> {
